...
public class EmployeesSteps {
    ...
    @Given("^The user have the id of the employee to delete$")
    public void theUserHaveTheIdOfTheEmployeeToDelete(DataTable employeeData) {
        List<Map<String, String>> data = employeeData.asMaps(String.class, String.class);
        employee = new EmployeeBuilder().withId(Integer.parseInt(data.get(0).get("id"))).create();
    }

    @When("^The user send a request to delete the employee by its id$")
    public void theUserSendARequestToDeleteTheEmployeeByItsId() {
        response = employeeControllerInstance.getDeleteEmployeeById(employee);
    }

    @And("^Affected rows of the response is \"([^\"]*)\"$")
    public void affectedRowsOfTheResponseIs(String rows) throws Throwable {
        ResponseData responseJson = JsonHelper.responteToJson(response);
        Assert.assertThat(String.format("Error: The affected row is: %s", rows),
                String.valueOf(responseJson.getAffectedRows()), Matchers.equalTo(String.valueOf(rows)));
    }
*/
}